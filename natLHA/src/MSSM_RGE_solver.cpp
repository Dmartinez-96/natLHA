...
std::vector<double> solveODEs(std::vector<double> initialConditions, double startTime, double endTime, double timeStep) {
    ...
    struct MyObserver{
        ...
        void operator()(const state_type& x, const double t) const {
            if (s->steps > 0) {
                ...
                if (s->steps == 1 || dt < s->minDt) { s->minDt = dt; s->minDtAt = t; }
            }
            ++(s->steps);
            s->prevT = t;
        }
    ...
    //
    // One line per solveODEs call, since a single naturalness evaluation makes several and
    // the interesting question is which of them is expensive.
